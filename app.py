...
class VideoProcessor:
    ...
    def recv(self, frame):
        ...
        if self.model and len(self.sequence) == 32 and (self.frame_counter % self.frame_skip == 0):
            try:
                tensor_kps = self.preprocessor.sample_and_pad(list(self.sequence)).to(self.device)
                with torch.no_grad():
...
def load_resources():
    ...
    try:
        ...
        if isinstance(raw_glosses, list):
            ...
            labels = {v: k for k, v in sorted_pairs} if isinstance(list(raw_glosses.values())[0], int) else \
                     {int(k): v for k, v in sorted(raw_glosses.items(), key=lambda x: int(x[0]))}
...
# --- VIDEO UPLOAD ---
    ...
    if uploaded_file:
        ...
        while cap.isOpened():
            ...
            if kp is not None: valid_frames.append(kp)
            
            curr += 1
            if total > 0: progress_bar.progress(min(curr/total, 1.0))
        ...
        if len(valid_frames) > 0:
            ...
            ph_label_ul.markdown(f"""
                <div class="big-label">Detected Sign:</div>
                <div class="big-pred {color_class}">{p_gloss}</div>
            """, unsafe_allow_html=True)
            
            ph_conf_ul.markdown(f"""
            ...
            for frame in original_frames:
                ...
                cv2.putText(frame_copy, f"{p_gloss} ({p_conf*100:.0f}%)", (10, 35), 
