...
```bash
data/wlasl_reduced/features_i3d_rgb_flow/
├── <gloss_label>/
...
```bash
python .\src\sign_language_model\scripts\extract_i3d_features.py --dataset-root .\data\wlasl_reduced\ --output-root .\data\wlasl_reduced\features_i3d_rgb_flow --crop-to-bbox --num-frames 32 --device cuda --streams rgb+flow
```
